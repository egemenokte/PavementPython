...
def iri(Y, segment_length, start_pos, step, box_filter = True, method = 2):
    ...
    """
        ...
        ZZ ... four-column matrix in the order of sprung mass displacement (m), unsprung mass displacement (m),
                station (m), Profile (m), IRI (m/km)
    """
    ...
    ZZ = np.zeros([end_si[-1]-start_si[0]+1,5])
...
#             DD[start_si[s]-start_si[0]+1:end_si[s]-start_si[0]+1] = D
            ...
            C1=np.multiply(Z[1:,1],np.diff(ft)/Y[-1,0])
            C2=np.multiply(Z[1:,3],np.diff(ft)/Y[-1,0])
            C1=Z[1:,0]/1000
            C2=Z[1:,2]/1000
            # plt.plot(C1+max(np.abs(C2)))
            # plt.plot(C2)
            DD[start_si[s]-start_si[0]+1:end_si[s]-start_si[0]+1] = D
            ZZ[start_si[s]-start_si[0]+1:end_si[s]-start_si[0]+1,0]=C1
            ZZ[start_si[s]-start_si[0]+1:end_si[s]-start_si[0]+1,1]=C2
            ZZ[start_si[s]-start_si[0]+1:end_si[s]-start_si[0]+1,2]=YC[si+1:ei+1,0]
            ZZ[start_si[s]-start_si[0]+1:end_si[s]-start_si[0]+1,3]=Y[1:,1]/1000
            ZZ[start_si[s]-start_si[0]+1:end_si[s]-start_si[0]+1,4]=D*len(D)
        z0 = Z[-1,:] 
    ...
    return IRI, equidistant,ZZ

import matplotlib.pyplot as plt
def plot_iri(IRI,YC, title):
#   PLOT_IRI IRI plot
#
...
#   YC ... input profile of the iri function
    ...
    ax1.set_ylabel('IRI [m/km]', color='b')
...
#   YC ... input profile of the iri function
    ...
    fig, ax1 = plt.subplots()
    lns1 = ax1.plot(ZZ[:,2], ZZ[:,3],  label = 'Road')
    lns2 = ax1.plot(ZZ[:,2], ZZ[:,1],  label = 'Unsprung')
    lns3 = ax1.plot(ZZ[:,2], ZZ[:,0],  label = 'Sprung')
    ax1.set_xlabel('Stationing [m]')
...
def readxlsx(path,stick=0.25): #stick in meters, size of filter
    ASP=pd.read_excel(path)
    road_profile=ASP.values
    #smooth to stick size
    x=np.arange(road_profile[0,0],road_profile[-1,0]+stick,stick)
    y=np.interp(np.arange(road_profile[0,0],road_profile[-1,0]+stick,stick),road_profile[:,0],road_profile[:,1])
    # dif=stick
    road_profile=np.column_stack([x,y])
    return road_profile
